    ...
    <div class="flex flex-1 mt-20">
        ...
        <main class="flex-1 p-6">
            <div class="max-w-2xl mx-auto bg-white shadow-lg rounded-2xl p-8">
                ...
                <form enctype="multipart/form-data" action="{{ route("edit.event",$data->event_id) }}" method="POST" class="space-y-5">
                    @csrf()
                    @method("PUT")
                    <div>
                ...
                <form method="POST" action="{{ route('hapus.galeri',$data->event_id)}}" class="mt-6">
                    @csrf
                    @method("DELETE")
                    <button 
